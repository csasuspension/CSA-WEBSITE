...
import {
  ...
  CreditCard, FileImage, Gift, Heart, LayoutDashboard, LogOut,
  Menu, Minus, PackageCheck, Phone, Plus, QrCode, Search, ShieldCheck,
  ShoppingBag, SlidersHorizontal, Sparkles, UserRound, Wrench, X,
} from "lucide-react";
...
const nav: { id: View; icon: typeof ShoppingBag }[] = [
  { id:"shop", icon:ShoppingBag }, { id:"garage", icon:Car },
  { id:"warranty", icon:ShieldCheck }, { id:"claims", icon:ClipboardCheck },
  { id:"promotions", icon:Gift }, { id:"admin", icon:LayoutDashboard },
];
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    safeRegister({
      ...
      description: "Navigate the visible portal to shop, garage, warranty, claims, promotions, or admin.",
      inputSchema: {
        type: "object",
  ...
  const labels = useMemo(() => ({ shop:t.shop, garage:t.garage, warranty:t.warranty, claims:t.claims, promotions:t.promotions, admin:t.admin }), [t]);
  const filtered = products.filter(p => (model === "ALL" || p.model === model) && (!query || `${p.name} ${p.model} ${p.id}`.toLowerCase().includes(query.toLowerCase())));
  const lines = Object.entries(cart).filter(([,q]) => q > 0).map(([id,qty]) => ({ product:products.find(p => p.id === id)!, qty }));
  ...
  return <div className="min-h-screen bg-[#090909] text-white">
    <header className="sticky top-0 z-40 bg-[#ffc400] text-black shadow-md">
      <div className="mx-auto flex h-[76px] max-w-[1600px] items-center px-4 sm:h-[88px] sm:px-7">
        <button className="flex items-center" onClick={() => go("shop")} aria-label="CSA home">
          <span className="font-display text-[38px] italic tracking-[-.07em] sm:text-5xl">CSA</span>
          <span className="ml-3 hidden border-l border-black/25 pl-3 text-[9px] font-black uppercase leading-tight tracking-[.16em] sm:block">High Performance<br/>Suspension</span>
        </button>
        <nav className="mx-auto hidden items-center gap-7 text-sm font-black lg:flex">
          {(["shop","warranty","claims","promotions"] as View[]).map(id=><button key={id} onClick={()=>go(id)} className={view===id?"underline decoration-2 underline-offset-8":"opacity-70 hover:opacity-100"}>{labels[id]}</button>)}
        </nav>
        <div className="ml-auto flex items-center gap-1 sm:gap-2">
          <button className="header-icon" onClick={()=>setLoginOpen(true)} aria-label={t.login}><UserRound/></button>
          ...
          <button className="header-icon hidden sm:grid" aria-label="Sign out"><LogOut/></button>
    ...
    <main className="mx-auto min-w-0 max-w-[1600px]">
        {view==="shop"&&<Shop lang={lang} t={t} model={model} setModel={setModel} products={filtered} add={add}/>}
...
function Shop({lang,t,model,setModel,products,add}:{lang:Lang;t:any;model:string;setModel:(v:string)=>void;products:Product[];add:(v:string)=>void}) {
  ...
  return <>
    ...
    <section className="grid grid-cols-3 bg-black text-center">
      {[[Wrench,lang==="th"?"โช้คอัพ":"SHOCKS"],[ShieldCheck,lang==="th"?"รับประกัน":"WARRANTY"],[BadgeCheck,lang==="th"?"ของแท้ CSA":"GENUINE CSA"]].map(([I,label],i)=>{const Icon=I as typeof Wrench;return <button key={label as string} className={`flex min-h-28 flex-col items-center justify-center gap-2 border-r border-white/10 px-2 text-xs font-black sm:text-base ${i===0?"border-t-4 border-t-[#ffc400] bg-white text-black":"text-white"}`}><Icon className="h-8 w-8"/>{label as string}</button>})}
    </section>
    ...
    <section className="bg-[#f2f2f2] px-5 py-12 text-black sm:px-10 sm:py-16">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{products.map(p=><article key={p.id} className="group overflow-hidden bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
        <div className="relative h-44 overflow-hidden bg-black">
          ...
          <button className="absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-full bg-white/90 text-black"><Heart className="h-4 w-4"/></button>
        </div>
...
function MenuDrawer({open,setOpen,lang,query,setQuery,labels,go}:{open:boolean;setOpen:(v:boolean)=>void;lang:Lang;query:string;setQuery:(v:string)=>void;labels:Record<View,string>;go:(v:View)=>void}) {
  return <Sheet open={open} onOpenChange={setOpen}><SheetContent className="w-[88vw] border-0 bg-white p-0 text-black sm:max-w-md" showCloseButton={false}>
    ...
    <div className="overflow-y-auto px-6 pb-8"><label className="my-6 flex items-center rounded-full border border-zinc-300 p-2 pl-5"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder={lang==="th"?"คำค้นหา":"Search"} className="h-10 min-w-0 flex-1 outline-none"/><span className="grid h-11 w-11 place-items-center rounded-full bg-black text-white"><Search/></span></label>
      ...
      <div className="mt-7 border-t border-zinc-300 pt-6"><a href="tel:020000000" className="flex items-center gap-3 text-lg font-black"><Phone/>Call 02-000-0000</a><button className="mt-6 flex items-center gap-3 text-lg font-black"><Heart/>รายการโปรด</button></div>
    </div>
  </SheetContent></Sheet>;
}
...
function Garage({lang}:{lang:Lang}) {
  return <Shell eyebrow="VEHICLE PROFILE" title={lang==="th"?"รถของฉัน":"My garage"}><div className="grid gap-4 xl:grid-cols-[1.1fr_.9fr]">
    ...
    <section className="border border-dashed border-white/20 p-5"><Wrench className="h-6 w-6 text-[#ffc400]"/><h3 className="mt-4 font-bold">{lang==="th"?"เพิ่มรถอีกคัน":"Add another vehicle"}</h3><p className="mt-1 text-sm text-zinc-500">{lang==="th"?"เก็บข้อมูลรถเพื่อค้นหาอะไหล่ที่ตรงรุ่นได้เร็วขึ้น":"Save your vehicle for faster fitment matching."}</p><Button variant="outline" className="mt-5 border-white/20 bg-transparent text-white">{lang==="th"?"เพิ่มข้อมูลรถ":"Add vehicle"}</Button></section>
  </div></Shell>;
}
...
function Warranty({lang,flash}:{lang:Lang;flash:(v:string)=>void}) {
  ...
  return <Shell eyebrow="SERIAL REGISTRATION" title={lang==="th"?"ลงทะเบียนรับประกัน":"Warranty registration"}><div className="grid gap-5 xl:grid-cols-[1fr_360px]">
    <section className="border border-white/10 bg-[#111] p-5 md:p-7">
      ...
      {step===1&&<><Label htmlFor="serial">Serial Number</Label><div className="mt-2 flex gap-2"><Input id="serial" value={serial} onChange={e=>setSerial(e.target.value.toUpperCase())} placeholder="2609 CSA 00001" className="border-white/15 bg-black/40 font-mono text-base"/><Button variant="outline" className="border-white/15 bg-transparent text-white"><QrCode className="h-4 w-4"/>SCAN</Button></div><p className={`mt-2 text-xs ${serial&&!valid?"text-red-400":"text-zinc-600"}`}>Format: 2609 CSA 00001</p><Button disabled={!valid} onClick={()=>setStep(2)} className="mt-6 bg-[#ffc400] text-black">{lang==="th"?"ตรวจสอบ Serial":"Verify serial"}<ChevronRight className="h-4 w-4"/></Button></>}
...
function Claims({lang,flash}:{lang:Lang;flash:(v:string)=>void}) {
  ...
  return <Shell eyebrow="CLAIM CENTER" title={lang==="th"?"แจ้งเคลมและติดตามสถานะ":"Claims & tracking"}><div className="grid gap-5 xl:grid-cols-[1fr_390px]">
    <section className="border border-white/10 bg-[#111] p-5 md:p-7">{done?<div className="py-8 text-center"><PackageCheck className="mx-auto h-12 w-12 text-[#ffc400]"/><h3 className="mt-4 text-2xl font-black">CSA-CLM-260920-001</h3><p className="mt-2 text-sm text-zinc-500">{lang==="th"?"ได้รับคำขอแล้ว เจ้าหน้าที่จะตรวจสอบหลักฐาน":"Claim received. Our team will review the evidence."}</p></div>:<div className="space-y-4"><div><Label>{lang==="th"?"เลือกสินค้าที่ลงทะเบียน":"Registered product"}</Label><select className="mt-2 h-10 w-full border border-white/15 bg-black/50 px-3 text-sm"><option>2609 CSA 00001 — HIACE 300 Front Pair</option></select></div><div><Label>{lang==="th"?"อาการหรือปัญหาที่พบ":"Issue description"}</Label><Textarea className="mt-2 min-h-28 border-white/15 bg-black/40" placeholder={lang==="th"?"อธิบายอาการ วันที่พบ และลักษณะการใช้งาน":"Describe the issue, date and usage conditions"}/></div><Upload label={lang==="th"?"เพิ่มรูปหรือวิดีโอหลักฐาน":"Add evidence files"}/><Button className="bg-[#ffc400] text-black" onClick={async()=>{await saveEvent("claim","CSA-CLM-260920-001",{serial:"2609 CSA 00001",product:"CSA Performance Front Pair"});setDone(true);flash(lang==="th"?"ส่งคำขอเคลม Demo แล้ว":"Demo claim submitted")}}>{lang==="th"?"ส่งคำขอเคลม":"Submit claim"}</Button></div>}</section>
    <aside className="border border-white/10 bg-[#111] p-5"><p className="text-xs font-bold tracking-widest text-[#ffc400]">ACTIVE CLAIM</p><h3 className="mt-2 font-black">CSA-CLM-260918-004</h3><p className="text-sm text-zinc-500">HIACE 300 · Rear Pair</p><div className="mt-6">{[["ส่งคำขอแล้ว","18 Sep",true],["กำลังตรวจสอบ","19 Sep",true],["รอผลอนุมัติ","—",false],["เสร็จสิ้น","—",false]].map(([a,b,c])=><div key={a as string} className="flex gap-3 pb-5"><span className={`mt-1 h-3 w-3 rounded-full border-2 ${c?"border-[#ffc400] bg-[#ffc400]":"border-zinc-700"}`}/><div><p className={`text-sm font-semibold ${c?"text-white":"text-zinc-600"}`}>{a as string}</p><p className="text-xs text-zinc-600">{b as string}</p></div></div>)}</div></aside>
  </div></Shell>;
}
...
function Admin({lang}:{lang:Lang}) {
  return <Shell eyebrow="OPERATIONS" title={lang==="th"?"ศูนย์จัดการ CSA":"CSA operations"}>
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[["12","Orders today",ShoppingBag],["4","Pending claims",ClipboardCheck],["28","Warranty registrations",ShieldCheck],["฿86,400","Demo revenue",CircleGauge]].map(([v,k,I])=>{const Icon=I as typeof ShoppingBag;return <div key={k as string} className="border border-white/10 bg-[#111] p-4"><Icon className="h-4 w-4 text-[#ffc400]"/><p className="mt-4 text-3xl font-black">{v as string}</p><p className="text-xs text-zinc-500">{k as string}</p></div>})}</div>
    <section className="mt-5 overflow-hidden border border-white/10 bg-[#111]"><div className="border-b border-white/10 p-4 font-bold">{lang==="th"?"คำขอเคลมล่าสุด":"Recent claims"}</div><div className="overflow-x-auto"><table className="w-full min-w-[700px] text-left text-sm"><thead className="bg-white/[.03] text-xs uppercase text-zinc-600"><tr><th className="p-4">Claim ID</th><th>Customer</th><th>Product</th><th>Status</th><th>Submitted</th></tr></thead><tbody>{[["CSA-CLM-260918-004","Somchai K.","Rear Pair / H300","Under review","18 Sep"],["CSA-CLM-260917-011","Narin P.","Front Pair / Majesty","Evidence needed","17 Sep"],["CSA-CLM-260915-008","Van Club CNX","Control Set / Commuter","Approved","15 Sep"]].map(r=><tr key={r[0]} className="border-t border-white/10"><td className="p-4 font-mono text-xs text-[#ffc400]">{r[0]}</td><td>{r[1]}</td><td className="text-zinc-400">{r[2]}</td><td><span className="border border-white/10 px-2 py-1 text-xs">{r[3]}</span></td><td className="text-zinc-500">{r[4]}</td></tr>)}</tbody></table></div></section>
  </Shell>;
}
...
function Checkout({open,setOpen,total,lang,done}:any) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogContent className="border-white/10 bg-[#121212] text-white sm:max-w-lg"><DialogHeader><DialogTitle className="font-display text-3xl uppercase">{lang==="th"?"ยืนยันคำสั่งซื้อ":"Confirm order"}</DialogTitle></DialogHeader><div className="space-y-4"><div className="grid grid-cols-2 gap-3"><Field label={lang==="th"?"ชื่อผู้รับ":"Recipient"} placeholder="Somchai"/><Field label={lang==="th"?"เบอร์โทร":"Phone"} placeholder="081-234-5678"/></div><div><Label>{lang==="th"?"ที่อยู่จัดส่ง":"Shipping address"}</Label><Textarea className="mt-2 border-white/15 bg-black/40" placeholder="Chiang Mai 50140"/></div><div><Label>{lang==="th"?"วิธีชำระเงิน":"Payment method"}</Label><div className="mt-2 grid grid-cols-2 gap-2"><Pay active={method==="promptpay"} onClick={()=>setMethod("promptpay")} icon={QrCode} title="PromptPay QR" sub="Upload slip · Free"/><Pay active={method==="gateway"} onClick={()=>setMethod("gateway")} icon={CreditCard} title="Payment Gateway" sub="Automatic · Demo"/></div></div><div className="flex items-end justify-between border-t border-white/10 pt-4"><span className="text-zinc-500">TOTAL</span><strong className="text-2xl">{money.format(total)}</strong></div><Button className="h-12 w-full bg-[#ffc400] text-black" onClick={done}>{lang==="th"?"สร้างออเดอร์ทดลอง":"Create demo order"}</Button></div></DialogContent></Dialog>;
}
