...
const modules:{id:Module;label:string;icon:typeof LayoutDashboard;count:string}[]=[
  {id:"dashboard",label:"Dashboard",icon:LayoutDashboard,count:""},
  {id:"crm",label:"CRM",icon:Users,count:"38"},
  {id:"sales",label:"Sales",icon:TrendingUp,count:"18"},
  {id:"orders",label:"Orders",icon:ShoppingBag,count:"12"},
  {id:"inventory",label:"Inventory",icon:Boxes,count:"3"},
  {id:"warranty",label:"Warranty & Claims",icon:ShieldCheck,count:"4"},
...
export function InternalSystem({lang}:{lang:Lang}){
  const [active,setActive]=useState<Module>("dashboard"); const [done,setDone]=useState<string[]>([]); const [serial,setSerial]=useState(""); const [toast,setToast]=useState("");
  const notify=(message:string)=>{setToast(message);window.setTimeout(()=>setToast(""),1800)};
  return <div className="min-h-[calc(100vh-88px)] bg-[#080808]"><div className="border-b border-white/10 bg-[#111] px-4 py-4 md:px-8"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-black tracking-[.2em] text-[#ffc400]">CSA COMPANY OS</p><h1 className="mt-1 text-2xl font-black">{lang==="th"?"ระบบบริหารงานภายใน":"Internal operations"}</h1></div><div className="flex items-center gap-3"><div className="text-right"><p className="text-sm font-bold">Site Owner</p><p className="text-xs text-zinc-500">21 Sep 2026 · Live</p></div><span className="grid h-10 w-10 place-items-center rounded-full bg-[#ffc400] font-black text-black">SO</span></div></div></div><div className="grid lg:grid-cols-[230px_1fr]"><aside className="border-b border-white/10 bg-[#0d0d0d] p-3 lg:min-h-[calc(100vh-165px)] lg:border-b-0 lg:border-r"><p className="px-3 pb-2 text-xs font-bold tracking-widest text-zinc-600">9 MODULES</p><div className="grid grid-cols-2 gap-1 sm:grid-cols-4 lg:grid-cols-1">{modules.map(m=>{const Icon=m.icon;return <button key={m.id} onClick={()=>setActive(m.id)} className={`flex min-w-0 items-center gap-3 px-3 py-3 text-left text-sm font-bold ${active===m.id?"bg-[#ffc400] text-black":"text-zinc-400 hover:bg-white/5 hover:text-white"}`}><Icon className="h-4 w-4 shrink-0"/><span className="truncate">{m.label}</span>{m.count&&<span className={`ml-auto text-xs ${active===m.id?"text-black/60":"text-zinc-600"}`}>{m.count}</span>}</button>})}</div><div className="mt-5 hidden border-t border-white/10 px-3 pt-5 lg:block"><p className="text-xs font-bold text-zinc-400">ONE RECORD RULE</p><p className="mt-2 text-xs leading-5 text-zinc-600">Customer → Order → Payment → Serial → Warranty history</p></div></aside><main className="min-w-0 p-4 md:p-7">{active==="dashboard"?<Dashboard setActive={setActive}/>:active==="admin"?<ProductAdmin/>:active==="website"?<WebsiteAdmin/>:<Queue module={active} done={done} onComplete={id=>{setDone(x=>[...x,id]);notify(lang==="th"?"ส่งงานไปยังขั้นตอนถัดไปแล้ว":"Work sent to the next module")}}/>}{active!=="admin"&&active!=="website"&&<section className="mt-6 border border-white/10 bg-[#111] p-4"><div className="flex flex-wrap items-center gap-3"><div className="mr-auto"><p className="font-bold">Product lifecycle search</p><p className="text-xs text-zinc-500">Serial → SKU → Order → Customer → Warranty → Claim</p></div><Input value={serial} onChange={e=>setSerial(e.target.value.toUpperCase())} placeholder="DEMO-SERIAL-043" className="w-full border-white/15 bg-black sm:w-56"/><Button onClick={()=>notify(serial?"Serial found · Warranty active · 0 open claims":"Enter a serial number")} className="bg-[#ffc400] text-black"><Search className="h-4 w-4"/>Search</Button></div>{serial&&<div className="mt-4 grid gap-3 border-t border-white/10 pt-4 text-sm sm:grid-cols-4"><Fact k="PRODUCT" v="HIACE 300 Rear Pair"/><Fact k="ORDER" v="DEMO-ORDER-006"/><Fact k="CUSTOMER" v="Demo Customer D"/><Fact k="WARRANTY" v="Active · 642 days"/></div>}</section>}</main></div>{toast&&<div className="fixed bottom-5 left-1/2 z-50 -translate-x-1/2 border border-[#ffc400]/40 bg-[#171717] px-4 py-3 text-sm font-bold shadow-2xl"><CheckCircle2 className="mr-2 inline h-4 w-4 text-[#ffc400]"/>{toast}</div>}</div>;
}
