...
export function SiteFooter({ lang, onNavigate }: { lang: Lang; onNavigate?:(view:View)=>void }) {
  ...
  return <>
    ...
    <footer className="bg-black px-6 py-12 text-white sm:px-10"><div className="mx-auto max-w-6xl"><h2 className="text-2xl font-black">{lang === "th" ? "ติดต่อ" : "Contact"}</h2><div className="mt-6 grid gap-4 sm:grid-cols-2"><a href={`tel:${content.contact.phone.replace(/\s/g,"")}`} className="flex items-center gap-3 text-lg font-black text-[#ffc400]"><Phone className="h-6 w-6 text-white"/>{content.contact.phone}</a><a href={`mailto:${content.contact.email}`} className="flex items-center gap-3 text-base font-bold"><Mail className="h-6 w-6"/>{content.contact.email}</a></div>
      <div className="mt-10 grid gap-9 border-b border-white/20 pb-10 md:grid-cols-3"><div><img src="/csa-logo.svg" alt="CSA High Performance Suspension" className="h-16 w-auto brightness-0 invert"/><p className="mt-4 max-w-sm text-sm leading-6 text-zinc-400">{lang === "th" ? "ผู้เชี่ยวชาญช่วงล่างสมรรถนะสูงสำหรับรถตู้ พร้อมบริการ รับประกัน และดูแลหลังการขาย" : "High-performance van suspension with warranty and after-sales support."}</p><div className="mt-5 flex gap-3"><a className="social-dot" href={content.contact.facebook} target="_blank" rel="noreferrer" aria-label="Facebook">f</a><a className="social-dot text-xs" href={content.contact.instagram} target="_blank" rel="noreferrer" aria-label="Instagram">IG</a><a className="social-dot text-xs" href={content.contact.line} target="_blank" rel="noreferrer" aria-label="LINE">LINE</a></div></div><FooterLinks onNavigate={onNavigate} title={lang === "th" ? "สินค้าและบริการ" : "Products"} items={products}/><FooterLinks onNavigate={onNavigate} title={lang === "th" ? "ดูแลลูกค้า" : "Support"} items={support}/></div>
