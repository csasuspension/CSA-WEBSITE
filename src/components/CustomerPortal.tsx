...
export function CustomerPortal() {
  ...
  const labels = useMemo<Record<View,string>>(() => lang==="th" ? ({shop:"สินค้า",member:"สมาชิก", "my-products":"สินค้าของฉัน",garage:"รถของฉัน",warranty:"รับประกัน",claims:"เคลม",promotions:"โปรโมชั่น",branches:"ตัวแทน",advice:"บทความ",about:"เกี่ยวกับ CSA",news:"ข่าวสาร",clips:"Clips",faq:"FAQ",careers:"ร่วมงานกับเรา",contact:"ติดต่อ",profile:"โปรไฟล์",admin:"ระบบภายใน"}) : ({shop:"Products",member:"Member", "my-products":"My products",garage:"My garage",warranty:"Warranty",claims:"Claims",promotions:"Promotions",branches:"Dealers",advice:"Advice",about:"About CSA",news:"News",clips:"Clips",faq:"FAQ",careers:"Careers",contact:"Contact",profile:"Profile",admin:"Internal system"}), [lang]);
  ...
  return <div className="min-h-screen overflow-x-clip bg-[#090909] text-white">
    <header className="sticky top-0 z-40 bg-[#ffc400] text-black shadow-md">
      <div className="mx-auto flex h-[48px] max-w-[1600px] items-center px-4 sm:h-[52px] sm:px-7">
        <button className="flex items-center" onClick={() => go("shop")} aria-label="CSA home">
          <CsaLogo className="h-7 sm:h-8" />
        </button>
        <nav className="mx-auto hidden items-center gap-7 text-xs font-semibold tracking-[-0.01em] lg:flex">
    ...
    <Checkout open={checkoutOpen} setOpen={setCheckoutOpen} total={total} lang={lang} items={lines.map(l=>({id:l.product.id,qty:l.qty}))} done={(orderId:string)=>{setCart({});setCheckoutOpen(false);flash(lang==="th"?`สร้างออเดอร์ ${orderId} แล้ว`:`Order ${orderId} created`)}}/>
          ...
          <div className="relative overflow-hidden bg-[#ffc400] p-6 sm:p-8 lg:min-h-[430px]">
            ...
            <div className="relative flex h-full flex-col">
              <div className="flex items-start justify-between gap-4 lg:block">
                ...
                <button onClick={clearFinder} className="rounded-full border border-black/25 px-4 py-2 text-xs font-semibold transition hover:bg-black hover:text-white lg:absolute lg:bottom-0 lg:left-0">{lang==="th"?"ล้าง":"Clear"}</button>
              </div>
              <div className="relative mt-6 lg:mt-10">
                ...
                <p className="mt-4 max-w-[240px] text-sm font-medium leading-6 opacity-70">{lang==="th"?"เลือกรถของคุณ":"Choose your vehicle"}</p>
              </div>
...
function ProductCard({product:p,lang,layout,favorite,toggleFavorite,details}:{product:Product;lang:Lang;layout:"grid"|"list";favorite:boolean;toggleFavorite:()=>void;details:()=>void}){
  ...
  return <article className={`group overflow-hidden rounded-[24px] bg-white shadow-sm ${layout==="list"?"grid grid-cols-[38%_1fr] sm:grid-cols-[260px_1fr]":""}`}>
    ...
    <div className="relative flex min-w-0 flex-col p-4 sm:p-5"><button onClick={toggleFavorite} aria-label={favorite?"Remove favorite":"Add favorite"} className="absolute right-3 top-3 text-zinc-400"><Heart className={`h-5 w-5 ${favorite?"fill-[#ffc400] text-[#b48b00]":""}`}/></button>
...
function MenuDrawer({open,setOpen,lang,query,setQuery,labels,go,content,loggedIn,memberName,login}:{open:boolean;setOpen:(v:boolean)=>void;lang:Lang;query:string;setQuery:(v:string)=>void;labels:Record<View,string>;go:(v:View)=>void;content:SiteContent;loggedIn:boolean;memberName:string;login:()=>void}) {
 const account=[{id:"my-products" as View,icon:Package},{id:"claims" as View,icon:ClipboardCheck}];
 const shopService=[{id:"branches" as View,icon:MapPin},{id:"warranty" as View,icon:ShieldCheck},{id:"promotions" as View,icon:Gift}];
 const helpContent=[{id:"advice" as View,icon:FileText},{id:"news" as View,icon:Newspaper},{id:"clips" as View,icon:PlayCircle},{id:"contact" as View,icon:Mail}];
 ...
 return <Sheet open={open} onOpenChange={setOpen}><SheetContent className="flex h-[100dvh] w-[88vw] flex-col border-0 bg-white p-0 text-black sm:max-w-md" showCloseButton={false}>
  <SheetHeader className="flex-row items-center justify-between border-b border-zinc-200 px-6 py-3.5"><SheetTitle className="text-2xl font-black">CSA</SheetTitle><button onClick={()=>setOpen(false)} aria-label="Close menu"><X className="h-7 w-7 text-zinc-500"/></button></SheetHeader>
  <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-6 pb-[calc(2rem+env(safe-area-inset-bottom))]">
   ...
   <button onClick={()=>go("shop")} className="flex w-full items-center justify-between rounded-xl bg-[#ffc400] px-4 py-4 text-left font-black"><span className="flex items-center gap-3"><ShockAbsorberIcon className="h-5 w-5"/>{lang==="th"?"เลือกโช้คอัพให้ตรงรุ่น":"Find shocks for my vehicle"}</span><ChevronRight className="h-5 w-5"/></button>
   <section className="mt-6"><p className="mb-2 text-xs font-semibold uppercase tracking-[.15em] text-zinc-400">{lang==="th"?"บัญชีของฉัน":"MY CSA"}</p>{loggedIn?<button onClick={()=>go("profile")} className="mb-2 flex w-full items-center gap-3 rounded-xl bg-zinc-100 p-4 text-left"><UserRound className="h-5 w-5"/><div><p className="font-black">{memberName||"CSA Member"}</p><p className="text-xs text-emerald-600">LINE CONNECTED</p></div></button>:<button onClick={()=>{setOpen(false);login()}} className="mb-2 flex w-full items-center justify-between rounded-xl bg-zinc-100 p-4 text-left font-black"><span className="flex items-center gap-3"><UserRound className="h-5 w-5"/>{lang==="th"?"LINE Login":"Sign in with LINE"}</span><ChevronRight className="h-5 w-5 text-zinc-400"/></button>}{account.map(x=>item(x.id,x.icon))}</section>
   <section className="mt-6"><p className="mb-2 text-xs font-semibold uppercase tracking-[.15em] text-zinc-400">{lang==="th"?"สินค้าและบริการ":"SHOP & SERVICE"}</p>{shopService.map(x=>item(x.id,x.icon))}</section>
   <section className="mt-6"><p className="mb-2 text-xs font-semibold uppercase tracking-[.15em] text-zinc-400">{lang==="th"?"ช่วยเหลือและเนื้อหา":"HELP & CONTENT"}</p>{helpContent.map(x=>item(x.id,x.icon))}</section>
   <div className="mt-7 border-t border-zinc-300 pt-6">{phone&&<a href={`tel:${phone.replace(/[^+\d]/g,"")}`} className="flex items-center gap-3 text-base font-black"><Phone/>{lang==="th"?"โทร":"Call"} {phone}</a>}{lineUrl&&<a href={lineUrl} target="_blank" rel="noreferrer" className="mt-5 flex items-center gap-3 text-base font-black"><MessageCircle className="h-5 w-5"/>{lang==="th"?"LINE CSA":"LINE CSA"}</a>}{!phone&&!lineUrl&&<button onClick={()=>go("contact")} className="flex items-center gap-3 text-base font-black"><Mail/>{lang==="th"?"ข้อมูลติดต่อ CSA":"CSA contact information"}</button>}</div>
  </div>
 </SheetContent></Sheet>;
}
...
function Checkout({open,setOpen,total,lang,items,done}:any) {
  const [customer,setCustomer]=useState(""),[phone,setPhone]=useState(""),[address,setAddress]=useState(""),[province,setProvince]=useState(""),[postcode,setPostcode]=useState(""),[note,setNote]=useState("");
  const [loading,setLoading]=useState(false),[error,setError]=useState("");
  const create=async()=>{setError("");if(!customer.trim()||!phone.trim()||!address.trim()){setError(lang==="th"?"กรอกชื่อ เบอร์โทร และที่อยู่ให้ครบ":"Enter name, phone and shipping address");return}setLoading(true);try{const response=await fetch("/api/orders",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({customer,phone,address,province,postcode,note,items})});const data=await response.json();if(!response.ok)throw new Error(data.error||"Could not create order");done(data.orderId)}catch(e){setError(e instanceof Error?e.message:"Could not create order");setLoading(false)}};
  return <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-h-[calc(100dvh-1rem)] overflow-y-auto border-white/10 bg-[#121212] pb-[calc(1.5rem+env(safe-area-inset-bottom))] text-white sm:max-w-lg"><DialogHeader><DialogTitle className="font-display text-3xl uppercase">{lang==="th"?"Checkout":"Checkout"}</DialogTitle></DialogHeader><div className="space-y-4"><div className="grid gap-3 sm:grid-cols-2"><div><Label>{lang==="th"?"ชื่อผู้รับ":"Recipient"}</Label><Input value={customer} onChange={e=>setCustomer(e.target.value)} className="mt-2 border-white/15 bg-black/40"/></div><div><Label>{lang==="th"?"เบอร์โทร":"Phone"}</Label><Input value={phone} onChange={e=>setPhone(e.target.value)} inputMode="tel" className="mt-2 border-white/15 bg-black/40"/></div></div><div><Label>{lang==="th"?"ที่อยู่จัดส่ง":"Shipping address"}</Label><Textarea value={address} onChange={e=>setAddress(e.target.value)} className="mt-2 min-h-24 border-white/15 bg-black/40"/></div><div className="grid grid-cols-2 gap-3"><div><Label>{lang==="th"?"จังหวัด":"Province"}</Label><Input value={province} onChange={e=>setProvince(e.target.value)} className="mt-2 border-white/15 bg-black/40"/></div><div><Label>{lang==="th"?"รหัสไปรษณีย์":"Postcode"}</Label><Input value={postcode} onChange={e=>setPostcode(e.target.value)} inputMode="numeric" className="mt-2 border-white/15 bg-black/40"/></div></div><div><Label>{lang==="th"?"หมายเหตุ":"Note"}</Label><Textarea value={note} onChange={e=>setNote(e.target.value)} className="mt-2 border-white/15 bg-black/40"/></div>{error&&<p className="rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm font-bold text-red-300">{error}</p>}<div className="flex items-end justify-between border-t border-white/10 pt-4"><span className="text-zinc-500">TOTAL</span><strong className="text-2xl">{money.format(total)}</strong></div><Button disabled={loading||!items.length||total<=0} className="h-12 w-full bg-[#ffc400] text-black" onClick={create}>{loading?(lang==="th"?"กำลังสร้างออเดอร์...":"Creating order..."):(lang==="th"?"ยืนยันออเดอร์":"Place order")}</Button><p className="text-center text-[10px] text-zinc-600">{lang==="th"?"ยังไม่ตัดเงิน ระบบชำระเงินจะเชื่อมในขั้นถัดไป":"No charge yet. Payment provider will be connected separately."}</p></div></DialogContent></Dialog>;
}
