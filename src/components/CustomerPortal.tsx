...
export function CustomerPortal() {
  ...
  return <div className="min-h-screen overflow-x-clip bg-[#090909] text-white">
    ...
    <Checkout open={checkoutOpen} setOpen={setCheckoutOpen} total={total} lang={lang} items={lines.map(l=>({id:l.product.id,qty:l.qty}))}/>
...
function Checkout({open,setOpen,total,lang,items}:any) {
  const [recipient,setRecipient]=useState(""); const [phone,setPhone]=useState(""); const [address,setAddress]=useState("");
  const [loading,setLoading]=useState(false); const [error,setError]=useState("");
  const pay=async()=>{setError("");if(!recipient.trim()||!phone.trim()||!address.trim()){setError(lang==="th"?"กรอกชื่อ เบอร์โทร และที่อยู่จัดส่งให้ครบ":"Enter recipient, phone and shipping address");return}setLoading(true);try{const response=await fetch("/api/checkout",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({recipient,phone,address,items})});const data=await response.json();if(!response.ok||!data.url)throw new Error(data.error||"Could not start payment");window.location.href=data.url}catch(e){setError(e instanceof Error?e.message:"Could not start payment");setLoading(false)}};
  return <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-h-[calc(100dvh-1rem)] overflow-y-auto border-white/10 bg-[#121212] pb-[calc(1.5rem+env(safe-area-inset-bottom))] text-white sm:max-w-lg"><DialogHeader><DialogTitle className="font-display text-3xl uppercase">{lang==="th"?"ชำระเงิน":"Checkout"}</DialogTitle></DialogHeader><div className="space-y-4"><div className="grid gap-3 sm:grid-cols-2"><div><Label>{lang==="th"?"ชื่อผู้รับ":"Recipient"}</Label><Input value={recipient} onChange={e=>setRecipient(e.target.value)} className="mt-2 border-white/15 bg-black/40"/></div><div><Label>{lang==="th"?"เบอร์โทร":"Phone"}</Label><Input value={phone} onChange={e=>setPhone(e.target.value)} inputMode="tel" className="mt-2 border-white/15 bg-black/40"/></div></div><div><Label>{lang==="th"?"ที่อยู่จัดส่ง":"Shipping address"}</Label><Textarea value={address} onChange={e=>setAddress(e.target.value)} className="mt-2 min-h-28 border-white/15 bg-black/40"/></div><div className="rounded-xl border border-white/10 bg-black/30 p-4"><div className="flex items-center gap-3"><CreditCard className="h-5 w-5 text-[#ffc400]"/><div><p className="text-sm font-bold">Stripe Checkout</p><p className="text-xs text-zinc-500">{lang==="th"?"เลือกวิธีชำระเงินที่ Stripe รองรับในหน้าถัดไป":"Choose an available payment method on the next screen"}</p></div></div></div>{error&&<p className="rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm font-bold text-red-300">{error}</p>}<div className="flex items-end justify-between border-t border-white/10 pt-4"><span className="text-zinc-500">TOTAL</span><strong className="text-2xl">{money.format(total)}</strong></div><Button disabled={loading||!items.length||total<=0} className="h-12 w-full bg-[#ffc400] text-black" onClick={pay}>{loading?(lang==="th"?"กำลังไป Stripe...":"Opening Stripe..."):(lang==="th"?"ชำระผ่าน Stripe":"Pay with Stripe")}</Button><p className="text-center text-[10px] text-zinc-600">{lang==="th"?"ระบบจะยืนยันออเดอร์เมื่อ Stripe แจ้งว่าชำระสำเร็จ":"Your order is marked paid only after Stripe confirms payment."}</p></div></DialogContent></Dialog>;
}
