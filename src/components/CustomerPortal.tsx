...
function MenuDrawer({open,setOpen,lang,query,setQuery,labels,go}:{open:boolean;setOpen:(v:boolean)=>void;lang:Lang;query:string;setQuery:(v:string)=>void;labels:Record<View,string>;go:(v:View)=>void}) {
  return <Sheet open={open} onOpenChange={setOpen}><SheetContent className="flex h-[100dvh] w-[88vw] flex-col border-0 bg-white p-0 text-black sm:max-w-md" showCloseButton={false}>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-6 pb-[calc(2rem+env(safe-area-inset-bottom))]"><label className="my-6 flex items-center rounded-full border border-zinc-300 p-2 pl-5"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder={lang==="th"?"คำค้นหา":"Search"} className="h-10 min-w-0 flex-1 outline-none"/><span className="grid h-11 w-11 place-items-center rounded-full bg-black text-white"><Search/></span></label>
